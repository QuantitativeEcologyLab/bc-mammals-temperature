...
d <- readRDS('data/tracking-data/rsf-data.rds') %>%
  ...
  pivot_longer(c(forest_perc, elevation_m, dist_water_m),
               names_to = 'variable') %>%
  mutate(
    species = gsub('southern mountain', '(s. mountain)', species) %>%
      gsub('boreal', '(boreal)', .),
    lab = COMMON_NAMES[map_int(species,
                               \(.s) which(SPECIES == .s))],
    variable =
      case_when(variable == 'forest_perc' ~ 'Forest cover (%)',
                variable == 'elevation_m' ~ 'Elevation (km)',
                variable == 'dist_water_m' ~ 'Distance from water (km)') %>%
