...
if(! file.exists('data/quadrature-data-2024-05-03.rds')) {
  d_0 <- mm %>%
    transmute(
      animal,
  ...
  d_0 <- readRDS('data/quadrature-data-2024-05-03.rds')
}
...
rsf <- bam(
  detected ~
    ...
    s(forest_perc, animal, by = species, k = 6, bs = 'fs', xt = list(bs = 'cr')) +
    s(elevation_m, animal, by = species, k = 6, bs = 'fs', xt = list(bs = 'cr')) +
    s(dist_water_m, animal, by = species, k = 6, bs = 'fs', xt = list(bs = 'cr')) +
  ...
  data = d_full,
  subset = animal %in% sample(unique(d_full$animal), 50)
  # weights = weight,
  method = 'fREML',
  discrete = TRUE,
